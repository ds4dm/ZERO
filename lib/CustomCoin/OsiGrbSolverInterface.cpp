...
void OsiGrbSolverInterface::solveFromSol() {
  ...
  GRBmodel *lp = getLpPtr(OsiGrbSolverInterface::KEEPCACHED_RESULTS);
  ...
  if (strength != OsiHintIgnore) {
	 ...
	 GUROBI_CALL("initialSolve",
					 GRBsetintparam(GRBgetenv(lp),
										 GRB_INT_PAR_METHOD,
										 takeHint ? GRB_METHOD_DUAL : GRB_METHOD_PRIMAL));
  }
  ...
  GUROBI_CALL(
		"initialSolve",
		GRBsetintparam(GRBgetenv(lp), GRB_INT_PAR_OUTPUTFLAG, (messageHandler()->logLevel() > 0)));

  /* Gabriele: Seed initial solution. */
  if (colsol_ != NULL && rowsol_ != NULL) {
	 int *   Pidx = new int[getNumCols()];
	 double *Pval = new double[getNumCols()];
	 int *   Didx = new int[getNumRows()];
	 double *Dval = new double[getNumRows()];
	 ...
	 GUROBI_CALL(
		  "resolve",
		  GRBsetdblattrlist(getMutableLpPtr(), GRB_DBL_ATTR_PSTART, getNumCols(), Pidx, Pval));
	 GUROBI_CALL(
		  "resolve",
		  GRBsetdblattrlist(getMutableLpPtr(), GRB_DBL_ATTR_DSTART, getNumRows(), Didx, Dval));
  ...
  if (stat == GRB_INF_OR_UNBD && presolve != GRB_PRESOLVE_OFF) {
	 GUROBI_CALL("initialSolve",
					 GRBsetintparam(GRBgetenv(lp), GRB_INT_PAR_PRESOLVE, GRB_PRESOLVE_OFF));
	 GUROBI_CALL("initialSolve", GRBoptimize(lp));
	 GUROBI_CALL("initialSolve", GRBsetintparam(GRBgetenv(lp), GRB_INT_PAR_PRESOLVE, presolve));
  }
...
void OsiGrbSolverInterface::branchAndBound() {
  ...
  if (colsol_ != NULL && domipstart) {
	 ...
	 GUROBI_CALL("branchAndBound",
					 GRBsetdblattrlist(
						  getMutableLpPtr(), GRB_DBL_ATTR_START, getNumIntegers(), discridx, discrval));
...
bool OsiGrbSolverInterface::setIntParam(OsiIntParam key, int value) {
  ...
  case OsiMaxNumIteration:
	 if (GRBsetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_ITERATIONLIMIT, (double)value) !=
		  0) {
		*messageHandler() << "Warning: Setting GUROBI iteration limit to" << value << "failed."
								<< CoinMessageEol;
		if (OsiGrbSolverInterface::globalenv_)
		  *messageHandler() << "\t GUROBI error message: "
								  << GRBgeterrormsg(OsiGrbSolverInterface::globalenv_) << CoinMessageEol;
		return false;
	 }
	 return true;
...
bool OsiGrbSolverInterface::setDblParam(OsiDblParam key, double value) {
  ...
  case OsiDualTolerance:
	 if (GRBsetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_OPTIMALITYTOL, value) != 0) {
		*messageHandler() << "Warning: Setting GUROBI dual (i.e., optimality) tolerance to" << value
								<< "failed." << CoinMessageEol;
		if (OsiGrbSolverInterface::globalenv_)
		  *messageHandler() << "\t GUROBI error message: "
								  << GRBgeterrormsg(OsiGrbSolverInterface::globalenv_) << CoinMessageEol;
		return false;
  ...
  case OsiPrimalTolerance:
	 if (GRBsetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_FEASIBILITYTOL, value) != 0) {
		*messageHandler() << "Warning: Setting GUROBI primal (i.e., feasiblity) tolerance to" << value
								<< "failed." << CoinMessageEol;
		if (OsiGrbSolverInterface::globalenv_)
		  *messageHandler() << "\t GUROBI error message: "
								  << GRBgeterrormsg(OsiGrbSolverInterface::globalenv_) << CoinMessageEol;
...
bool OsiGrbSolverInterface::setStrParam(OsiStrParam key, const std::string &value) {
  ...
  case OsiProbName:
	 GUROBI_CALL(
		  "setStrParam",
		  GRBsetstrattr(getLpPtr(), GRB_STR_ATTR_MODELNAME, const_cast<char *>(value.c_str())));
	 return true;
...
bool OsiGrbSolverInterface::setHintParam(OsiHintParam    key,
  ...
  if (key == OsiDoScale) {
	 GUROBI_CALL("setHintParam",
					 GRBsetintparam(GRBgetenv(getMutableLpPtr()), GRB_INT_PAR_SCALEFLAG, yesNo));
...
bool OsiGrbSolverInterface::getIntParam(OsiIntParam key, int &value) const {
  ...
  case OsiMaxNumIteration:
	 ...
	 GUROBI_CALL("getIntParam",
					 GRBgetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_ITERATIONLIMIT, &dblval));
	 value = (int)dblval;
	 return true;
...
bool OsiGrbSolverInterface::getDblParam(OsiDblParam key, double &value) const {
  ...
  switch (key) {
	 ...
	 //   	  break;
  case OsiDualTolerance:
	 GUROBI_CALL("getDblParam",
					 GRBgetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_OPTIMALITYTOL, &value));
	 return true;
  case OsiPrimalTolerance:
	 GUROBI_CALL("getDblParam",
					 GRBgetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_FEASIBILITYTOL, &value));
	 return true;
  case OsiObjOffset:
...
bool OsiGrbSolverInterface::getHintParam(OsiHintParam     key,
  ...
  if (key == OsiDoScale) {
	 ...
	 int value;
	 GUROBI_CALL("getHintParam",
					 GRBgetintparam(GRBgetenv(getMutableLpPtr()), GRB_INT_PAR_SCALEFLAG, &value));
	 yesNo = value;
	 return true;
  }
...
bool OsiGrbSolverInterface::isAbandoned() const {
  ...
  return (stat == GRB_LOADED || stat == GRB_NUMERIC || stat == GRB_INTERRUPTED);
}
...
bool OsiGrbSolverInterface::isProvenPrimalInfeasible() const {
  ...
  int stat;
  GUROBI_CALL("isProvenPrimalInfeasible",
				  GRBgetintattr(getMutableLpPtr(), GRB_INT_ATTR_STATUS, &stat));
...
bool OsiGrbSolverInterface::isProvenDualInfeasible() const {
  ...
  int stat;
  GUROBI_CALL("isProvenDualInfeasible",
				  GRBgetintattr(getMutableLpPtr(), GRB_INT_ATTR_STATUS, &stat));
...
bool OsiGrbSolverInterface::isIterationLimitReached() const {
  ...
  int stat;
  GUROBI_CALL("isIterationLimitReached",
				  GRBgetintattr(getMutableLpPtr(), GRB_INT_ATTR_STATUS, &stat));
...
#ifdef UNUSED
// below we assume that getBasisStatus uses the same values as the warm start enum, i.e. 0: free, 1:
// basic, 2: upper, 3: lower
static CompileTimeAssert<CoinWarmStartBasis::isFree == 0> UNUSED
	 change_In_Enum_CoinWarmStartBasis_Status_free_breaks_this_function;
static CompileTimeAssert<CoinWarmStartBasis::basic == 1> UNUSED
	 change_In_Enum_CoinWarmStartBasis_Status_basic_breaks_this_function;
static CompileTimeAssert<CoinWarmStartBasis::atUpperBound == 2> UNUSED
	 change_In_Enum_CoinWarmStartBasis_Status_upper_breaks_this_function;
static CompileTimeAssert<CoinWarmStartBasis::atLowerBound == 3> UNUSED
	 change_In_Enum_CoinWarmStartBasis_Status_lower_breaks_this_function;
#endif
...
CoinWarmStart *OsiGrbSolverInterface::getWarmStart() const {
  ...
  GUROBI_CALL("getWarmStart",
				  GRBgetintattrarray(getMutableLpPtr(), GRB_INT_ATTR_VBASIS, 0, numcols, cstat));
  GUROBI_CALL("getWarmStart",
				  GRBgetintattrarray(getMutableLpPtr(), GRB_INT_ATTR_CBASIS, 0, numrows, rstat));
  ...
  for (i = 0; i < numrows; ++i) {
	 ...
	 case GRB_NONBASIC_UPPER:
		GUROBI_CALL("getWarmStart",
						GRBgetcharattrelement(getMutableLpPtr(), GRB_CHAR_ATTR_SENSE, i, &sense));
		ws->setArtifStatus(
			 i, (sense == '>' ? CoinWarmStartBasis::atUpperBound : CoinWarmStartBasis::atLowerBound));
		break;
...
bool OsiGrbSolverInterface::setWarmStart(const CoinWarmStart *warmstart) {
  ...
  GUROBI_CALL("setWarmStart",
				  GRBsetintattrarray(getLpPtr(OsiGrbSolverInterface::FREECACHED_RESULTS),
											GRB_INT_ATTR_CBASIS,
											0,
											numrows,
											stat));
...
void OsiGrbSolverInterface::markHotStart() {
  ...
  if (numcols > hotStartCStatSize_) {
	 ...
	 hotStartCStatSize_ = static_cast<int>(
		  1.2 *
		  static_cast<double>(numcols + nauxcols)); // get some extra space for future hot starts
	 hotStartCStat_ = new int[hotStartCStatSize_];
  }
  if (numrows > hotStartRStatSize_) {
	 delete[] hotStartRStat_;
	 hotStartRStatSize_ = static_cast<int>(
		  1.2 * static_cast<double>(numrows)); // get some extra space for future hot starts
	 hotStartRStat_ = new int[hotStartRStatSize_];
  }

  GUROBI_CALL("markHotStart", GRBupdatemodel(getMutableLpPtr()));

  GUROBI_CALL("markHotStart",
				  GRBgetintattrarray(
						getMutableLpPtr(), GRB_INT_ATTR_VBASIS, 0, numcols + nauxcols, hotStartCStat_));
  GUROBI_CALL(
		"markHotStart",
		GRBgetintattrarray(getMutableLpPtr(), GRB_INT_ATTR_CBASIS, 0, numrows, hotStartRStat_));
}
...
void OsiGrbSolverInterface::solveFromHotStart() {
  ...
  GUROBI_CALL("solveFromHotStart",
				  GRBgetdblparam(GRBgetenv(getMutableLpPtr()), GRB_DBL_PAR_ITERATIONLIMIT, &maxiter));
  GUROBI_CALL("solveFromHotStart",
				  GRBsetdblparam(GRBgetenv(getMutableLpPtr()),
									  GRB_DBL_PAR_ITERATIONLIMIT,
									  (double)hotStartMaxIteration_));
  ...
  if (collower_ == NULL) {
	 ...
	 if (ncols > 0) {
		...
		if (nauxcols)
		  GUROBI_CALL(
				"getColLower",
				GRBgetdblattrlist(getMutableLpPtr(), GRB_DBL_ATTR_LB, ncols, colmap_O2G, collower_));
		else
		  GUROBI_CALL("getColLower",
						  GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_LB, 0, ncols, collower_));
	 }
  }
  ...
  if (colupper_ == NULL) {
	 ...
	 if (ncols > 0) {
		...
		if (nauxcols)
		  GUROBI_CALL(
				"getColUpper",
				GRBgetdblattrlist(getMutableLpPtr(), GRB_DBL_ATTR_UB, ncols, colmap_O2G, colupper_));
		else
		  GUROBI_CALL("getColUpper",
						  GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_UB, 0, ncols, colupper_));
	 }
  }
...
const char *OsiGrbSolverInterface::getRowSense() const {
  ...
  if (rowsense_ == NULL) {
	 ...
	 if (nrows > 0) {
		...
		GUROBI_CALL("getRowSense",
						GRBgetcharattrarray(getMutableLpPtr(), GRB_CHAR_ATTR_SENSE, 0, nrows, rowsense_));
...
const double *OsiGrbSolverInterface::getRightHandSide() const {
  ...
  if (rhs_ == NULL) {
	 ...
	 if (nrows > 0) {
		...
		GUROBI_CALL("getRightHandSide",
						GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_RHS, 0, nrows, rhs_));

		/* for ranged rows, we give the rhs of the aux. variable used to represent the range of this
		 * row as row rhs */
		if (nauxcols)
		  for (int i = 0; i < nrows; ++i)
			 if (auxcolind[i] >= 0)
				GUROBI_CALL(
					 "getRightHandSide",
					 GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_UB, auxcolind[i], &rhs_[i]));
	 }
  }
	 ...
	 if (nauxcols) {
		...
		for (int i = 0; i < nrows; ++i)
		  if (auxcolind[i] >= 0) {
			 GUROBI_CALL(
				  "getRowRange",
				  GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_LB, auxcolind[i], &auxlb));
			 GUROBI_CALL(
				  "getRowRange",
				  GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_UB, auxcolind[i], &auxub));
  ...
  if (rowlower_ == NULL) {
	 ...
	 if (nrows > 0) {
		...
		for (int i = 0; i < nrows; ++i) {
		  if (auxcolind && auxcolind[i] >= 0) {
			 assert(rowsense[i] == 'R');
			 GUROBI_CALL("getRowLower",
							 GRBgetdblattrelement(
								  getMutableLpPtr(), GRB_DBL_ATTR_LB, auxcolind[i], &rowlower_[i]));
		  } else {
			 GUROBI_CALL("getRowLower",
							 GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_RHS, i, &rhs));
  ...
  if (rowupper_ == NULL) {
	 ...
	 if (nrows > 0) {
		...
		for (int i = 0; i < nrows; ++i) {
		  if (auxcolind && auxcolind[i] >= 0) {
			 assert(rowsense[i] == 'R');
			 GUROBI_CALL("getRowUpper",
							 GRBgetdblattrelement(
								  getMutableLpPtr(), GRB_DBL_ATTR_UB, auxcolind[i], &rowupper_[i]));
		  } else {
			 GUROBI_CALL("getRowUpper",
							 GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_RHS, i, &rhs));
  ...
  if (obj_ == NULL) {
	 ...
	 if (ncols > 0) {
		...
		if (nauxcols)
		  GUROBI_CALL(
				"getObjCoefficients",
				GRBgetdblattrlist(getMutableLpPtr(), GRB_DBL_ATTR_OBJ, ncols, colmap_O2G, obj_));
		else
		  GUROBI_CALL("getObjCoefficients",
						  GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_OBJ, 0, ncols, obj_));
	 }
  }
...
const CoinPackedMatrix *OsiGrbSolverInterface::getMatrixByRow() const {
  ...
  if (matrixByRow_ == NULL) {
	 ...
	 GUROBI_CALL("getMatrixByRow",
					 GRBgetconstrs(getMutableLpPtr(), &nelems, NULL, NULL, NULL, 0, nrows));
	 ...
	 GUROBI_CALL("getMatrixByRow",
					 GRBgetconstrs(getMutableLpPtr(), &nelems, starts, indices, elements, 0, nrows));

	 matrixByRow_ = new CoinPackedMatrix();

	 // Should be able to pass null for length of packed matrix,
	 // assignMatrix does not seem to allow (even though documentation say it is possible to do
	 // this). For now compute the length.
	 starts[nrows] = nelems;
	 for (int i = 0; i < nrows; ++i)
...
const CoinPackedMatrix *OsiGrbSolverInterface::getMatrixByCol() const {
  ...
  if (matrixByCol_ == NULL) {
	 ...
	 if (ncols > 0) {
		...
		GUROBI_CALL("getMatrixByCol",
						GRBgetvars(getMutableLpPtr(), &nelems, NULL, NULL, NULL, 0, ncols + nauxcols));
		...
		GUROBI_CALL(
			 "getMatrixByCol",
			 GRBgetvars(getMutableLpPtr(), &nelems, starts, indices, elements, 0, ncols + nauxcols));
...
const double *OsiGrbSolverInterface::getColSolution() const {
  ...
  if (colsol_ == NULL) {
	 ...
	 if (ncols > 0) {
		...
		if (GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_X, 0, colsol_) ==
			 0) { // if a solution is available, get it
		  if (nauxcols)
			 GUROBI_CALL(
				  "getColSolution",
				  GRBgetdblattrlist(getMutableLpPtr(), GRB_DBL_ATTR_X, ncols, colmap_O2G, colsol_));
		  else
			 GUROBI_CALL("getColSolution",
							 GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_X, 0, ncols, colsol_));
		} else {
		  *messageHandler()
				<< "Warning: OsiGrbSolverInterface::getColSolution() called, but no solution "
					"available! Returning lower bounds, but they may be at -infinity. Be aware!"
				<< CoinMessageEol;
		  if (OsiGrbSolverInterface::globalenv_)
			 *messageHandler() << "\t GUROBI error message: "
									 << GRBgeterrormsg(OsiGrbSolverInterface::globalenv_) << CoinMessageEol;
		  if (nauxcols)
			 GUROBI_CALL(
				  "getColSolution",
				  GRBgetdblattrlist(getMutableLpPtr(), GRB_DBL_ATTR_LB, ncols, colmap_O2G, colsol_));
		  else
			 GUROBI_CALL("getColSolution",
							 GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_LB, 0, ncols, colsol_));
		}
	 }
  }
  ...
  if (rowsol_ == NULL) {
	 ...
	 if (nrows > 0) {
		...
		if (GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_PI, 0, rowsol_) == 0) {
		  GUROBI_CALL("getRowPrice",
						  GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_PI, 0, nrows, rowsol_));
		} else {
		  *messageHandler() << "Warning: OsiGrbSolverInterface::getRowPrice() called, but no "
									  "solution available! Returning 0.0. Be aware!"
								  << CoinMessageEol;
		  if (OsiGrbSolverInterface::globalenv_)
			 *messageHandler() << "\t GUROBI error message: "
									 << GRBgeterrormsg(OsiGrbSolverInterface::globalenv_) << CoinMessageEol;
...
const double *OsiGrbSolverInterface::getReducedCost() const {
  ...
  if (redcost_ == NULL) {
	 ...
	 if (ncols > 0) {
		...
		if (GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_RC, 0, redcost_) ==
			 0) { // if reduced costs are available, get them
		  if (nauxcols)
			 GUROBI_CALL(
				  "getReducedCost",
				  GRBgetdblattrlist(getMutableLpPtr(), GRB_DBL_ATTR_RC, ncols, colmap_O2G, redcost_));
		  else
			 GUROBI_CALL("getReducedCost",
							 GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_RC, 0, ncols, redcost_));
		} else {
		  *messageHandler() << "Warning: OsiGrbSolverInterface::getReducedCost() called, but no "
									  "solution available! Returning 0.0. Be aware!"
								  << CoinMessageEol;
		  if (OsiGrbSolverInterface::globalenv_)
			 *messageHandler() << "\t GUROBI error message: "
  ...
  if (rowact_ == NULL) {
	 ...
	 if (nrows > 0) {
		...
		if (GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_SLACK, 0, rowact_) == 0) {
		  GUROBI_CALL("getRowActivity",
						  GRBgetdblattrarray(getMutableLpPtr(), GRB_DBL_ATTR_SLACK, 0, nrows, rowact_));

		  for (int r = 0; r < nrows; ++r) {
			 if (nauxcols && auxcolind[r] >= 0) {
				GUROBI_CALL(
					 "getRowActivity",
					 GRBgetdblattrelement(getMutableLpPtr(), GRB_DBL_ATTR_X, auxcolind[r], &rowact_[r]));
			 } else
				rowact_[r] = getRightHandSide()[r] - rowact_[r];
		  }
		} else {
		  *messageHandler() << "Warning: OsiGrbSolverInterface::getRowActivity() called, but no "
									  "solution available! Returning 0.0. Be aware!"
								  << CoinMessageEol;
		  if (OsiGrbSolverInterface::globalenv_)
			 *messageHandler() << "\t GUROBI error message: "
...
double OsiGrbSolverInterface::getObjValue() const {
  ...
  if (GRBgetdblattr(getMutableLpPtr(), GRB_DBL_ATTR_OBJVAL, &objval) == 0) {
	 ...
	 *messageHandler() << "Warning: OsiGrbSolverInterface::getObjValue() called, but probably no "
								 "solution available! Returning 0.0. Be aware!"
							 << CoinMessageEol;
	 if (OsiGrbSolverInterface::globalenv_)
		*messageHandler() << "\t GUROBI error message: "
...
int OsiGrbSolverInterface::getIterationCount() const {
  ...
  GUROBI_CALL("getIterationCount",
				  GRBgetdblattr(getMutableLpPtr(), GRB_DBL_ATTR_ITERCOUNT, &itercnt));

  return (int)itercnt;
}

//------------------------------------------------------------------
std::vector<double *> OsiGrbSolverInterface::getDualRays(int maxNumRays, bool fullRay) const {
  debugMessage(
		"OsiGrbSolverInterface::getDualRays(%d,%s)\n", maxNumRays, fullRay ? "true" : "false");

  if (fullRay == true) {
	 throw CoinError("Full dual rays not yet implemented.", "getDualRays", "OsiGrbSolverInterface");
  }
...
void OsiGrbSolverInterface::setObjCoeff(int elementIndex, double elementValue) {
  ...
  GUROBI_CALL("setObjCoeff",
				  GRBsetdblattrelement(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM),
											  GRB_DBL_ATTR_OBJ,
											  nauxcols ? colmap_O2G[elementIndex] : elementIndex,
											  elementValue));
...
void OsiGrbSolverInterface::setObjCoeffSet(const int *   indexFirst,
  ...
  if (nauxcols) {
	 ...
	 GUROBI_CALL("setObjCoeffSet",
					 GRBsetdblattrlist(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM),
											 GRB_DBL_ATTR_OBJ,
											 cnt,
											 indices,
...
void OsiGrbSolverInterface::setColSetBounds(const int *   indexFirst,
  ...
  GUROBI_CALL("setColSetBounds",
				  GRBsetdblattrlist(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM),
										  GRB_DBL_ATTR_LB,
										  cnt,
										  indices ? indices : const_cast<int *>(indexFirst),
										  lbList));
  GUROBI_CALL("setColSetBounds",
				  GRBsetdblattrlist(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM),
										  GRB_DBL_ATTR_UB,
...
void OsiGrbSolverInterface::setRowType(int i, char sense, double rightHandSide, double range) {
  debugMessage(
		"OsiGrbSolverInterface::setRowType(%d, %c, %g, %g)\n", i, sense, rightHandSide, range);
  ...
  if (nauxcols && auxcolind[i] >= 0) { // so far, row i is a ranged row
	 ...
	 GUROBI_CALL(
		  ...
		  GRBsetcharattrelement(
				getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM), GRB_CHAR_ATTR_SENSE, i, grbsense));
	 GUROBI_CALL("setRowType",
					 GRBsetdblattrelement(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM),
												 GRB_DBL_ATTR_RHS,
												 i,
...
void OsiGrbSolverInterface::setRowSetTypes(const int *   indexFirst,
  ...
  for (int i = 0; i < cnt; ++i) {
	 ...
	 if (nauxcols && auxcolind[indexFirst[i]] >= 0) { // so far, row is a ranged row
		...
		case 'R':
		  ...
		  GUROBI_CALL("setRowSetTypes",
						  GRBsetdblattrelement(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM),
													  GRB_DBL_ATTR_LB,
													  auxcolind[indexFirst[i]],
													  rhsList[i] - rangeList[i]));
		  GUROBI_CALL("setRowSetTypes",
						  GRBsetdblattrelement(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM),
													  GRB_DBL_ATTR_UB,
													  auxcolind[indexFirst[i]],
  ...
  GUROBI_CALL("setRowSetTypes",
				  GRBsetcharattrlist(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_ROW),
											GRB_CHAR_ATTR_SENSE,
											cnt,
											const_cast<int *>(indexFirst),
											grbsense));
  GUROBI_CALL("setRowSetTypes",
				  GRBsetdblattrlist(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_ROW),
										  GRB_DBL_ATTR_RHS,
										  cnt,
										  const_cast<int *>(indexFirst),
										  rhs));
...
void OsiGrbSolverInterface::setContinuous(int index) {
  ...
  if (probtypemip_) {
	 ...
	 GUROBI_CALL("setContinuous",
					 GRBsetcharattrelement(getMutableLpPtr(),
												  GRB_CHAR_ATTR_VTYPE,
												  nauxcols ? colmap_O2G[index] : index,
												  GRB_CONTINUOUS));
  }
}
...
void OsiGrbSolverInterface::setRowName(int ndx, std::string name) {
  ...
  GUROBI_CALL("setRowName",
				  GRBsetstrattrelement(
						getLpPtr(), GRB_STR_ATTR_CONSTRNAME, ndx, const_cast<char *>(name.c_str())));
}
...
void OsiGrbSolverInterface::setColName(int ndx, std::string name) {
  ...
  GUROBI_CALL("setColName",
				  GRBsetstrattrelement(getLpPtr(),
											  GRB_STR_ATTR_VARNAME,
											  nauxcols ? colmap_O2G[ndx] : ndx,
...
void OsiGrbSolverInterface::setObjSense(double s) {
  ...
  GUROBI_CALL("setObjSense",
				  GRBsetintattr(getLpPtr(OsiGrbSolverInterface::FREECACHED_RESULTS),
...
void OsiGrbSolverInterface::addCol(const CoinPackedVectorBase &vec,
  ...
  GUROBI_CALL("addCol",
				  GRBaddvar(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_ROW),
								vec.getNumElements(),
...
void OsiGrbSolverInterface::addCols(const int                          numcols,
  ...
  GUROBI_CALL("addCols",
				  GRBaddvars(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_ROW),
								 numcols,
								 nz,
								 start,
								 index,
...
void OsiGrbSolverInterface::deleteCols(const int num, const int *columnIndices) {
  ...
  if (nauxcols) {
	 ...
	 GUROBI_CALL("deleteCols",
					 GRBdelvars(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_ROW), num, ind));
	 ...
	 for (int i = ind[0]; i < nc + nauxcols; ++i) {
		...
		if (colmap_G2O[i + offset] >= 0) { // if variable at (hithero) position i+offset was a regular
													  // variable, then it is now stored at position i
		...
		} else { // if variable at (hithero) position i+offset was an auxiliary variable, then the
					// corresponding row index is now stored at position i
	 ...
	 GUROBI_CALL("deleteCols",
					 GRBdelvars(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_ROW),
									num,
...
void OsiGrbSolverInterface::addRow(const CoinPackedVectorBase &vec,
											  ...
											  const double                rowrng) {
  debugMessage(
		"OsiGrbSolverInterface::addRow(%p, %c, %g, %g)\n", (void *)&vec, rowsen, rowrhs, rowrng);
  ...
  GUROBI_CALL("addRow",
				  GRBaddconstr(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_COLUMN),
									vec.getNumElements(),
									const_cast<int *>(vec.getIndices()),
									const_cast<double *>(vec.getElements()),
									grbsense,
...
void OsiGrbSolverInterface::addRows(const int                          numrows,
  ...
  GUROBI_CALL("addRows",
				  GRBaddconstrs(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_ROW),
									 numrows,
									 nz,
									 start,
									 index,
...
void OsiGrbSolverInterface::deleteRows(const int num, const int *rowIndices) {
  ...
  if (nauxcols) { // check if a ranged row should be deleted; if so, then convert it into a normal
						// row first
	 for (int i = 0; i < num; ++i) {
  ...
  GUROBI_CALL("deleteRows",
				  GRBdelconstrs(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_COLUMN),
									 num,
...
void OsiGrbSolverInterface::loadProblem(const CoinPackedMatrix &matrix,
  ...
  if (!matrix.isColOrdered()) {
	 ...
	 // up to GUROBI 2.0.1, GUROBI may give an "Index is out of range" error if the constraint matrix
	 // has uninitalized "gaps"
#if (GRB_VERSION_MAJOR < 2) ||                                                                     \
	 (GRB_VERSION_MAJOR == 2 && GRB_VERSION_MINOR == 0 && GRB_VERSION_TECHNICAL <= 1)
  if (m->hasGaps()) {
  ...
  GUROBI_CALL("loadProblem",
				  GRBgetintattr(getMutableLpPtr(), GRB_INT_ATTR_MODELSENSE, &modelsense));
  ...
  GUROBI_CALL("loadProblem",
				  GRBloadmodel(getEnvironmentPtr(),
									&lp_,
									const_cast<char *>(pn.c_str()),
									nc,
									nr,
									modelsense,
									0.0,
  ...
  // GUROBI up to version 2.0.1 may return a scaled LP after GRBoptimize when requesting it via
  // GRBgetvars
#if (GRB_VERSION_MAJOR < 2) ||                                                                     \
	 (GRB_VERSION_MAJOR == 2 && GRB_VERSION_MINOR == 0 && GRB_VERSION_TECHNICAL <= 1)
...
void OsiGrbSolverInterface::loadProblem(const int     numcols,
													 const int     numrows,
													 const int *   start,
													 const int *   index,
													 const double *value,
...
void OsiGrbSolverInterface::loadProblem(const int     numcols,
													 ...
													 const double *rowrng) {
  debugMessage(
		"OsiGrbSolverInterface::loadProblem(4)(%d, %d, %p, %p, %p, %p, %p, %p, %p, %p, %p)\n",
		numcols,
		numrows,
		(void *)start,
		(void *)index,
...
// Read mps files
//-----------------------------------------------------------------------------
int OsiGrbSolverInterface::readMps(const char *filename, const char *extension) {
...
void OsiGrbSolverInterface::writeMps(const char *filename,
  ...
  // 2. the instance in Gurobi may have extra columns due to reformulated ranged rows, that may be
  // confusing
...
bool OsiGrbSolverInterface::isDemoLicense() const {
  ...
  GUROBI_CALL("isDemoLicense",
				  GRBnewmodel(env, &testlp, "licensetest", 1000, NULL, NULL, NULL, NULL, NULL));
...
unsigned int OsiGrbSolverInterface::getNumInstances() { return numInstances_; }
...
OsiGrbSolverInterface::OsiGrbSolverInterface(bool use_local_env)
	 : OsiSolverInterface(), localenv_(NULL), lp_(NULL), hotStartCStat_(NULL), hotStartCStatSize_(0),
		hotStartRStat_(NULL), hotStartRStatSize_(0),
		hotStartMaxIteration_(1000000), // ??? default iteration limit for strong branching is large
		nameDisc_(0), obj_(NULL), collower_(NULL), colupper_(NULL), rowsense_(NULL), rhs_(NULL),
		rowrange_(NULL), rowlower_(NULL), rowupper_(NULL), colsol_(NULL), rowsol_(NULL),
		redcost_(NULL), rowact_(NULL), matrixByRow_(NULL), matrixByCol_(NULL), probtypemip_(false),
		domipstart(false), colspace_(0), coltype_(NULL), nauxcols(0), auxcolspace(0),
		colmap_O2G(NULL), colmap_G2O(NULL), auxcolindspace(0), auxcolind(NULL) {
...
OsiGrbSolverInterface::OsiGrbSolverInterface(GRBenv *localgrbenv)
	 : OsiSolverInterface(), localenv_(localgrbenv), lp_(NULL), hotStartCStat_(NULL),
		hotStartCStatSize_(0), hotStartRStat_(NULL), hotStartRStatSize_(0),
		hotStartMaxIteration_(1000000), // ??? default iteration limit for strong branching is large
		nameDisc_(0), obj_(NULL), collower_(NULL), colupper_(NULL), rowsense_(NULL), rhs_(NULL),
		rowrange_(NULL), rowlower_(NULL), rowupper_(NULL), colsol_(NULL), rowsol_(NULL),
		redcost_(NULL), rowact_(NULL), matrixByRow_(NULL), matrixByCol_(NULL), probtypemip_(false),
  ...
  if (localenv_ == NULL) { // if user called this constructor with NULL pointer, we assume that he
									// meant that a local environment should be created
	 GUROBI_CALL("OsiGrbSolverInterface", GRBloadenv(&localenv_, NULL));
	 assert(localenv_ != NULL);
  }
...
OsiGrbSolverInterface::OsiGrbSolverInterface(const OsiGrbSolverInterface &source)
	 : OsiSolverInterface(source), localenv_(NULL), lp_(NULL), hotStartCStat_(NULL),
		hotStartCStatSize_(0), hotStartRStat_(NULL), hotStartRStatSize_(0),
		hotStartMaxIteration_(source.hotStartMaxIteration_), nameDisc_(source.nameDisc_), obj_(NULL),
		collower_(NULL), colupper_(NULL), rowsense_(NULL), rhs_(NULL), rowrange_(NULL),
		rowlower_(NULL), rowupper_(NULL), colsol_(NULL), rowsol_(NULL), redcost_(NULL), rowact_(NULL),
		matrixByRow_(NULL), matrixByCol_(NULL), probtypemip_(false), domipstart(false), colspace_(0),
		coltype_(NULL), nauxcols(0), auxcolspace(0), colmap_O2G(NULL), colmap_G2O(NULL),
		auxcolindspace(0), auxcolind(NULL) {
...
OsiSolverInterface::ApplyCutsReturnCode OsiGrbSolverInterface::applyCuts(const OsiCuts &cs,
  ...
  GUROBI_CALL("applyRowCuts",
				  GRBaddrangeconstrs(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_COLUMN),
											nToApply,
  ...
  if (nNewVars != nTrulyRanged) {
	 ...
	 std::cerr << "ERROR in applying cuts for Gurobi: " << __FILE__ << " : "
				  << "line " << __LINE__ << " . Exiting" << std::endl;
	 exit(-1);
  }
...
GRBmodel *OsiGrbSolverInterface::getMutableLpPtr() const {
  if (lp_ == NULL) {
	 ...
	 GUROBI_CALL(
		  "getMutableLpPtr",
		  GRBnewmodel(getEnvironmentPtr(), &lp_, "OsiGrb_problem", 0, NULL, NULL, NULL, NULL, NULL));
	 assert(lp_ != NULL);
...
void OsiGrbSolverInterface::convertToRangedRow(int rowidx, double rhs, double range) {
  ...
  double minusone = -1.0;
  GUROBI_CALL("convertToRangedRow",
				  GRBaddvar(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_PROBLEM),
								1,
								&rowidx,
								&minusone,
								0.0,
...
void OsiGrbSolverInterface::convertToNormalRow(int rowidx, char sense, double rhs) {
  ...
  GUROBI_CALL("convertToNormalRow",
				  GRBdelvars(getLpPtr(OsiGrbSolverInterface::KEEPCACHED_ROW), 1, &auxcolind[rowidx]));
...
int OsiGrbSolverInterface::canDoSimplexInterface() const { return 0; }
...
bool OsiGrbSolverInterface::basisIsAvailable() const {
  ...
  if (status == GRB_LOADED || status == GRB_INFEASIBLE || status == GRB_INF_OR_UNBD ||
...
void OsiGrbSolverInterface::getBasisStatus(int *cstat, int *rstat) const {
  ...
  if (nauxcols)
	 GUROBI_CALL(
		  "getBasisStatus",
		  GRBgetintattrlist(getMutableLpPtr(), GRB_INT_ATTR_VBASIS, numcols, colmap_O2G, cstat));
  else
	 GUROBI_CALL("getBasisStatus",
					 GRBgetintattrarray(getMutableLpPtr(), GRB_INT_ATTR_VBASIS, 0, numcols, cstat));
  ...
  for (int i = 0; i < numrows; ++i) {
	 if (nauxcols && auxcolind[i] >= 0) { /* if the row is a ranged row, then we take the basis
			 status from the corresponding auxiliary column is this correct??? */
		int auxcolstat;
		GUROBI_CALL(
			 "getBasisStatus",
			 GRBgetintattrelement(getMutableLpPtr(), GRB_INT_ATTR_VBASIS, auxcolind[i], &auxcolstat));
		switch (auxcolstat) {
		case GRB_BASIC:
		  rstat[i] = 1;
		  break;
