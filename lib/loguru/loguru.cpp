	 ...
	 for (int arg_it = 1; arg_it < argc; ++arg_it) {
		...
		if (strncmp(cmd, verbosity_flag, arg_len) == 0 &&
		  ...
		  if (value_str[0] == '\0') {
			 ...
			 arg_it += 1;
			 CHECK_LT_F(
				  arg_it, argc, "Missing verbosiy level after " LOGURU_FMT(s) "", verbosity_flag);
		  ...
		  if (req_verbosity != Verbosity_INVALID) {
			 ...
			 CHECK_F(
				  ...
				  "Invalid verbosity. Expected integer, INFO, WARNING, ERROR or OFF, got '" LOGURU_FMT(
						s) "'",
				  value_str);
		  }
		} else {
...
#define getcwd _getcwd
...
#ifdef _WIN32
	 strncat_s(
		  buff, buff_size - strlen(buff) - 1, s_argv0_filename.c_str(), buff_size - strlen(buff) - 1);
  ...
  */
  ...
  Verbosity get_verbosity_from_name(const char *name) {
	 auto verbosity =
		  s_name_to_verbosity_callback ? (*s_name_to_verbosity_callback)(name) : Verbosity_INVALID;
  ...
  bool remove_callback(const char *id) {
	 std::lock_guard<std::recursive_mutex> lock(s_mutex);
	 auto                                  it = std::find_if(
        begin(s_callbacks), end(s_callbacks), [&](const Callback &c) { return c.id == id; });
...
// Tell the OS the thread name
  ...
  buffer[0] = 0;
	 ...
	 const auto thread_id = std::hash<std::thread::id>{}(std::this_thread::get_id());
		...
		if (right_align_hex_id) {
		  snprintf(
				buffer, length, "%*X", static_cast<int>(length - 1), static_cast<unsigned>(thread_id));
		} else {
  ...
  static void print_preamble_header(char *out_buff, size_t out_buff_size) {
	 ...
	 if (g_preamble_file && pos < out_buff_size) {
		pos += snprintf(
			 out_buff + pos, out_buff_size - pos, "%*s:line  ", LOGURU_FILENAME_WIDTH, "file");
	 }
  ...
  static void print_preamble(
		char *out_buff, size_t out_buff_size, Verbosity verbosity, const char *file, unsigned line) {
	 if (out_buff_size == 0) {
		return;
	 }
	 out_buff[0] = '\0';
	 ...
	 if (g_preamble_uptime && pos < out_buff_size) {
		pos += snprintf(out_buff + pos, out_buff_size - pos, "(%8.3fs) ", uptime_sec);
	 }
	 if (g_preamble_thread && pos < out_buff_size) {
		pos += snprintf(
			 out_buff + pos, out_buff_size - pos, "[%-*s]", LOGURU_THREADNAME_WIDTH, thread_name);
	 }
	 if (g_preamble_file && pos < out_buff_size) {
  ...
  static void
  log_message(int stack_trace_skip, Message &message, bool with_indentation, bool abort_if_fatal) {
...
#if LOGURU_USE_FMTLIB
  template <typename... Args> std::string vstrprintf(const char *format, const Args &... args) {
	 auto        text   = textprintf(format, args...);
	 std::string result = text.c_str();
	 return result;
  }

  template <typename... Args> std::string strprintf(const char *format, const Args &... args) {
