...
# Version history
	...
	* Version 1.7.2 - 2018-04-22 - Fix a bug where large file names could cause stack corruption
(thanks @ccamporesi)
	* Version 1.8.0 - 2018-04-23 - Shorten long file names to keep preamble fixed width
	* Version 1.9.0 - 2018-09-22 - Adjust terminal colors, add LOGURU_VERBOSE_SCOPE_ENDINGS, add
LOGURU_SCOPE_TIME_PRECISION, add named log levels
	* Version 2.0.0 - 2018-09-22 - Split loguru.hpp into loguru.hpp and loguru.cpp
	* Version 2.1.0 - 2019-09-23 - Update fmtlib + add option to loguru::init to NOT set main thread
name.
...
// Semantic versioning. Loguru version can be printed with printf("%d.%d.%d", LOGURU_VERSION_MAJOR,
...
#undef MAX
  ...
  */
  ...
  LOGURU_EXPORT extern bool g_preamble_header; // Prepend each log start by a descriptions line with
  ...
  */
  ...
  LOGURU_EXPORT
  void
  log(Verbosity verbosity, const char *file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, ...)
  ...
  log_and_abort(int stack_trace_skip, const char *expr, const char *file, unsigned line);
  ...
  /* Returns the thread name for this thread.
	  On most *nix systems this will return the system thread name (settable from both within and
	  without Loguru). On other systems it will return whatever you set in `set_thread_name()`; If no
	  thread name is set, this will return a hexadecimal thread id. `length` should be the number of
	  bytes available in the buffer. 17 is a good number for length. `right_align_hex_id` means any
  ...
  */
  ...
  // template<typename Printer>
  // EcEntryLambda<Printer> make_ec_entry_lambda(const char* file, unsigned line, const char* descr,
  // Printer&& printer)
  // {
...
#define CHECK_WITH_INFO_F(test, info, ...)                                                         \
  ...
  ? (void)0                                                                                        \
  : loguru::log_and_abort(0, "CHECK FAILED:  " info "  ", __FILE__, __LINE__, ##__VA_ARGS__)
...
#define CHECK_OP_F(expr_left, expr_right, op, ...)                                                 \
  do {                                                                                             \
	 ...
	 if (!LOGURU_PREDICT_TRUE(val_left op val_right)) {                                             \
		...
		auto fail_info =                                                                             \
			 loguru::textprintf("CHECK FAILED:  " LOGURU_FMT(s) " " LOGURU_FMT(s) " " LOGURU_FMT(     \
		...
		loguru::log_and_abort(                                                                       \
			 0, fail_info.c_str(), __FILE__, __LINE__, LOGURU_FMT(s), user_msg.c_str());              \
	 }                                                                                              \
  } while (false)
