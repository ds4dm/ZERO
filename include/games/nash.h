...
namespace Game {
  ...
  public: // Constructors
	 ...
	 /// constraints
	 explicit NashGame(GRBEnv *e, std::vector<std::shared_ptr<QP_Param>> players, arma::sp_mat MC,
	                   arma::vec MCRHS, unsigned int nLeadVar = 0, arma::sp_mat leadA = {},
	                   arma::vec leadRHS = {});
	 ...
	 const NashGame &formulateLCP(arma::sp_mat &M, arma::vec &q, perps &Compl,
	                              bool writeToFile = false, std::string M_name = "dat/LCP.txt",
	 ...
	 bool isSolved(const arma::vec &sol, unsigned int &violPlayer, arma::vec &violSol,
