...
void MathOpt::getDualMembershipLP(
	 std::unique_ptr<GRBModel> &convexModel, ///<[in/out] the model object
	 unsigned int &             numV,        ///[in/out] the number of vertices already in the model
	 const arma::sp_mat &       V,           ///<[in] the vertices matrix
	 unsigned int &             numR,        ///<[in/out] the number rays already in the model
	 const arma::sp_mat &       R,           ///<[in] the rays matrix
	 const arma::vec &          vertex,      ///<[in] the vertex input that needs to be verifies
	 const bool &               normalization) {
  /**
	* @brief Given a vector @param R of rays, and @param V or vertices, builds a model in @param
	* ConvexModel that certifies whether @param vertex belongs to the convex-hull generated by @param
	* V and @param R. In case @param numV and/or @param numR are specified, it just updates the model
	* in @param ConvexModel with the missing vertices and rays. In case @param normalization is
	* specified, the model includes a normalization constraint.
	*/
